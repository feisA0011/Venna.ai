...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      const response = await fetch(`/api/dashboard/analytics/roi?venueId=${VENUE_ID}`);
    ...
    () => [
      {
        ...
        value: analytics ? analytics.roi.hoursSavedThisMonth.toFixed(1) : "0.0"
      },
      {
        label: "Estimated € saved",
        value: analytics ? `€${analytics.roi.estimatedEurSavedThisMonth.toFixed(0)}` : "€0"
      },
      {
        label: "Escalation SLA met",
        value: analytics
          ? `${analytics.roi.escalationSlaPercent.toFixed(0)}% (${analytics.roi.resolvedEscalations} resolved)`
          : "0%"
      }
    ],
    [analytics]
    ...
    <section className="grid gap-6">
      <div className="rounded-2xl border border-neutral-200 p-6">
        ...
        <p className="mt-2 text-sm text-neutral-600">
          Aggregated, privacy-safe ROI analytics from venue conversations (no raw user text).
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-neutral-200 p-6">
        ...
        <div className="mt-4">
          ...
          <ul className="mt-2 space-y-1 text-sm text-neutral-700">
            {(analytics?.topQuestionTopics ?? []).map((item) => (
              <li key={item.topic}>
