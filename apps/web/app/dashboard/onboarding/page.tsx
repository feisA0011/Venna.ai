...
function OnboardingClient() {
  ...
  const refresh = async () => {
    const [stateRes, docsRes] = await Promise.all([
      fetch(`/api/dashboard/onboarding?venueId=${venueId}&userId=${userId}`),
      fetch(`/api/dashboard/knowledge/documents?venueId=${venueId}&status=pending&userId=${userId}`)
    ]);

    if (!stateRes.ok) {
      const error = await stateRes.json();
      setMessage(error.error ?? "Unable to load onboarding state.");
      return;
    }
  ...
  const runIngestion = async () => {
    ...
    const response = await fetch("/api/dashboard/knowledge/ingestion/start", {
      method: "POST",
      headers: { "content-type": "application/json", "x-venna-user-id": userId },
      body: JSON.stringify({ venueId, url: websiteUrlInput })
    });

    if (!response.ok) {
      const error = await response.json();
      setMessage(error.error ?? "Ingestion failed.");
      return;
    }
  ...
  const reviewDoc = async (documentId: string, status: "approved" | "rejected") => {
    const response = await fetch(`/api/dashboard/knowledge/documents/${documentId}/review`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-venna-user-id": userId },
    ...
    if (!response.ok) {
      const error = await response.json();
      setMessage(error.error ?? "Document review failed.");
      return;
    }
    await refresh();
  };
  ...
  const toggleGoLive = async (enabled: boolean) => {
    const response = await fetch("/api/dashboard/onboarding", {
      method: "POST",
      headers: { "content-type": "application/json", "x-venna-user-id": userId },
      body: JSON.stringify({ venueId, action: "setGoLive", enabled })
    });
    if (!response.ok) {
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-2xl border border-neutral-200 p-4 space-y-2">
        ...
        <Link href={`/widget-test?venueId=${venueId}`} className="text-sm text-blue-700 underline">Open widget test page</Link>
      </div>
...
export default function OnboardingPage() {
  return (
    <Suspense fallback={<section className="text-sm text-neutral-600">Loading onboarding…</section>}>
      <OnboardingClient />
    </Suspense>
  );
}
